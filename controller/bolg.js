...
exports.viewone_by_blogcategory = async(req, res)=>{
    const viewonebycategory = await blog.find({blog_category: req.params.id})
    .populate("blog_category");
    if(viewonebycategory){
        res.status(200).json({
            status: true,
            msg: "success",
            length: viewonebycategory.length,
            data: viewonebycategory,
        })
    }else{
        res.status(400).json({
            status: false,
