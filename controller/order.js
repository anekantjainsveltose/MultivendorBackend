...
exports.allorder_list = async (req, res) => {
  const getalldata = await orderproduct
    .find()
    .sort({ sortorder: 1 })
    .populate("product")
    .populate("attribute")
...
exports.viewone_order = async (req, res) => {
  try {
    const vieworder_data = await orderproduct
...
exports.pending_order = async (req, res) => {
  const findpending_order = await orderproduct
    .find({ status: "pending" })
    .populate("product")
    .populate("attribute")
...
exports.total_order = async (req, res) => {
  const finddata = await orderproduct
    .countDocuments()
    .then((finddata) => {
      res.status(200).json({
        status: true,
...
exports.customer_order_list = async (req, res) => {
  const findexist = await orderproduct
...
exports.assinged_product_list = async (req, res) => {
  const findexist = await orderproduct
    .find({ assing_drive: req.params.id })
    .populate("product")
    .populate("assing_drive");
  if (findexist) {
    res.status(200).json({
...
exports.assinged_product_viewone_list = async (req, res) => {
  const findviewone = await orderproduct
    .findOne({ _id: req.params.id })
    .populate("product")
    .populate("assing_drive");
  if (findviewone) {
...
exports.top_selling = async (req, res) => {
  const topselling_data = await orderproduct
    .find()
  ...
  if (topselling_data) {
    res.status(200).json({
      status: true,
      msg: "success",
      length: topselling_data.length,
      data: topselling_data,
    });
  } else {
    res.status(400).Json({
      status: false,
