...
exports.assinged_product_list = async(req, res)=>{
    const findexist = await orderproduct.find({assing_drive: req.params.id})
    .populate("product")
    .populate("assing_drive")
    if(findexist){
        res.status(200).json({
...
exports.assinged_product_viewone_list = async(req, res)=>{
    const findviewone = await orderproduct.findOne({_id: req.params.id})
    .populate("product")
    .populate("assing_drive")
    if(findviewone){
        res.status(200).json({
...
exports.top_selling = async(req, res)=>{
    const topselling_data = await orderproduct.find().sort({Status: "complete"});
    if(topselling_data){
        res.status(200).json({
            status: true,
            msg: "success",
