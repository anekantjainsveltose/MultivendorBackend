...
exports.addsubcategory = async (req, res) => {
  ...
  const findexist = await subproductcategories.findOne({
    subcategory_name: subcategory_name,
  });
  ...
  if (findexist) {
    ...
    const { image, thumbnail_img, webbanner, app_banner } = req.files;
    ...
    if (image) {
      ...
      newsubcategory.image = image_array[0];
    }
    if (thumbnail_img) {
      thumbnail_img_array = [];
      for (let i = 0; i < thumbnail_img.length; i++) {
        const resp = await cloudinary.uploader.upload(thumbnail_img[i].path);
        fs.unlinkSync(thumbnail_img[i].path);
        thumbnail_img_array.push(resp.secure_url);
      }
      newsubcategory.thumbnail_img = thumbnail_img_array[0];
    }
    if (webbanner) {
      alluploads = [];
      ...
      newsubcategory.webbanner = alluploads[0];
    }
    if (app_banner) {
      alluploads = [];
      ...
      newsubcategory.app_banner = alluploads[0];
    }
    newsubcategory
      .save()
      .then((data) => {
...
exports.getalldata = async (req, res) => {
  const findall = await subproductcategories
    .find()
    .sort({
      sortorder: 1,
    })
...
exports.del_subcategory = async (req, res) => {
  const deleteone = await subproductcategories.deleteOne({
    _id: req.params.id,
  });
  if (deleteone) {
    res.status(200).json({
...
exports.edit_Subcategory = async (req, res) => {
  ...
  await subproductcategories
    .findOneAndUpdate({ _id: req.params.id }, { $set: data }, { new: true })
