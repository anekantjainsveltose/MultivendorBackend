...
exports.vender_veryfyotp = async (req, res) => {
    ...
    if (getuser) {
        if (otp == 1234) {
            await appvender.findOneAndUpdate(
                {
                    _id: getuser._id,
                },
                { $set: { status: "true" } },
...
exports.vender_register_img = async (req, res) => {
    const { adhar_no, pancard_no, account_no, cus_name, branch, ifsc_code, vendoor_img,adhar_img_front,adhar_img_back,pancard_img_front,pancard_img_back ,passbook_img} = req.body
    ...
    if (vendoor_img) {
        ...
        if(adhar_img_front){
        if (adhar_img_front) {
            const base64Data = new Buffer.from(adhar_img_front.replace(/^data:image\/\w+;base64,/, ""), 'base64')
            detectMimeType(base64Data);
    ...
    if(adhar_img_back){
        if (adhar_img_back) {
            const base64Data = new Buffer.from(adhar_img_back.replace(/^data:image\/\w+;base64,/, ""), 'base64')
            detectMimeType(base64Data);
    ...
    if(pancard_img_front){
        if (pancard_img_front) {
            const base64Data = new Buffer.from(pancard_img_front.replace(/^data:image\/\w+;base64,/, ""), 'base64')
            detectMimeType(base64Data);
    ...
    if(pancard_img_back){
        if (pancard_img_back) {
            const base64Data = new Buffer.from(pancard_img_back.replace(/^data:image\/\w+;base64,/, ""), 'base64')
            detectMimeType(base64Data);
    ...
    if(passbook_img){
        if (passbook_img) {
            const base64Data = new Buffer.from(passbook_img.replace(/^data:image\/\w+;base64,/, ""), 'base64')
            detectMimeType(base64Data);
